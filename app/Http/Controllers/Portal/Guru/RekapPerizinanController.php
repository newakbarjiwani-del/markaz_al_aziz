...
class RekapPerizinanController extends BaseController
{
    ...
    {
        $classes = $this->guruTeachingScope()->classLabels();

        return view('admin.perizinan.rekap-laporan', [
            'title' => 'Rekap Perizinan Siswa',
            'schools' => [],
            'classes' => $classes,
            'ajaxUrl' => route('portal.guru.rekap-perizinan.data'),
        ]);
    }
    ...
    public function data(Request $request): JsonResponse
    {
        ...
        $classIds = $this->guruTeachingScope()->classIds();
        ...
        if ($classIds !== []) {
            $query->whereHas('siswa', fn ($q) => $q->whereIn('kelas_id', $classIds));
        } elseif ($guru->sekolah_id) {

...
<x-admin.datatable-page
    ...
    subtitle="Daftar pelanggaran anak"
    :ajax-url="route('portal.ortu.pelanggaran-siswa.data')"
...
(function () {
    ...
    function getShowUrl(row) {
        return '{{ route("portal.ortu.pelanggaran-siswa.show", "__ID__") }}'.replace('__ID__', row.id);
    }
        ...
        table.on('click', '.pelanggaran-detail-btn', function () {
            ...
            $.getJSON(getShowUrl(row), function (res) {
                ...
                if (typeof window.renderBuktiDetailList === 'function') {
                    window.renderBuktiDetailList(buktiList[0], d.bukti || [], { group: 'portal-ortu-pelanggaran-' + (d.id || Date.now()) });
                } else {

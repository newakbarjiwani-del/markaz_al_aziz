...
<x-admin.datatable-page
    title="Izin Keluar Masuk (Sekolah)"
    subtitle="Perizinan siswa meninggalkan area sekolah harian pada jam pelajaran / KBM"
    :ajax-url="$ajaxUrl ?? route('portal.perizinan.keluar-masuk.data')"
    export-filename="Izin Keluar Masuk Harian"
    :columns="['Siswa', 'Alasan', 'Pemberi Izin', 'Waktu Berangkat', 'Batas Kembali', 'Kembali Aktual', 'Status', 'Aksi']"
    ...
    <x-slot:actions>
        <button type="button" class="btn-primary flex-1 sm:flex-none"
                ...
                data-store-url="{{ $storeUrl ?? route('portal.perizinan.keluar-masuk.store') }}"
                data-modal-title="Tambah Izin Keluar Masuk">
            <x-icon name="plus" size="sm" class="mr-1" /> Tambah Perizinan
        </button>
    </x-slot:actions>
    <x-slot:filters>
...
@include('admin.perizinan.partials.form-modal', ['jenis' => \App\Models\Perizinan::JENIS_KELUAR_MASUK])

...
<x-admin.datatable-page
    ...
    subtitle="Kelola pencatatan pelanggaran siswa"
    :ajax-url="route('admin.prestasi-pelanggaran.pelanggaran-siswa.data')"
    ...
    <x-slot:actions>
        <button type="button" class="btn-primary flex-1 sm:flex-none"
                ...
                data-store-url="{{ route('admin.prestasi-pelanggaran.pelanggaran-siswa.store') }}"
...
@include('admin.partials.prestasi-pelanggaran-form-modal', ['type' => 'pelanggaran', 'context' => 'siswa', 'katalogOptions' => $katalogOptions])
...
(function () {
    document.addEventListener('edit-record-populated', function (e) {
        ...
        $.getJSON(record.show_url, function (res) {
            ...
            if (typeof window.showExistingBukti === 'function') {
                window.showExistingBukti('pelanggaran-siswa-modal', d.bukti);
            }
        });
    });
    ...
    document.addEventListener('click', function (e) {
        ...
        $.getJSON(btn.dataset.viewUrl, function (res) {
            ...
            if (typeof window.renderBuktiDetailList === 'function') {
                window.renderBuktiDetailList(buktiList[0], d.bukti || [], { group: 'pelanggaran-siswa-' + (d.id || Date.now()) });
            } else {
    ...
    document.addEventListener('modalReset', function (e) {
        if (e.detail?.formId === 'pelanggaran-siswa-form') {
            var form = document.getElementById('pelanggaran-siswa-form');
            form.action = '{{ route("admin.prestasi-pelanggaran.pelanggaran-siswa.store") }}';
